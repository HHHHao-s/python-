...
def getlist(id,type=0):# 0为获取歌手名,1为不获取(获取歌手名用时长)
    print('processing...')
    response = requests.get("https://music.163.com/playlist?id=" + id, headers = headers)
    response.encoding = 'utf-8'
    s = etree.HTML(response.text)
    list_name = s.xpath('//*[@name="keywords"]/@content')[0].split('，')[0]
    music_id = list(map(lambda x:x.split('=')[-1],s.xpath('//ul[@class="f-hide"]/li/a/@href')))
    music_name = s.xpath('//ul[@class="f-hide"]/li/a/text()')
    music_artist = []
    for id in music_id:
        if type == 0:
            artist = etree.HTML(requests.get('https://music.163.com/song?id=' + id,headers = headers).text).xpath('//*[@property="og:music:artist"]/@content')[0]
            music_artist.append(artist)
        if type == 1:
            music_artist.append('')
    play_list = [(x, music_id[music_name.index(x)],music_artist[music_name.index(x)]) for x in music_name]
